\section{寄存器描述}
\regover{
{\hyperref[i2s-i2s-config]{i2s\_config}}&
\\
\hline
{\hyperref[i2s-i2s-int-sts]{i2s\_int\_sts}}&
\\
\hline
{\hyperref[i2s-i2s-bclk-config]{i2s\_bclk\_config}}&
\\
\hline
{\hyperref[i2s-i2s-fifo-config-0]{i2s\_fifo\_config\_0}}&
\\
\hline
{\hyperref[i2s-i2s-fifo-config-1]{i2s\_fifo\_config\_1}}&
\\
\hline
{\hyperref[i2s-i2s-fifo-wdata]{i2s\_fifo\_wdata}}&
\\
\hline
{\hyperref[i2s-i2s-fifo-rdata]{i2s\_fifo\_rdata}}&
\\
\hline
{\hyperref[i2s-i2s-io-config]{i2s\_io\_config}}&
\\
\hline
}
...
\\\hline
11:9&RSVD& & & \\\hline
8:7&cr\_fs\_ch\_cnt&r/w&2'd0&Channel count of each frame \par 2'd0: FS 2-channel mode \par 2'd1: FS 3-channel mode (DSP mode only) \par 2'd2: FS 4-channel mode (DSP mode only) \par 2'd3: FS 6-channel mode (DSP mode only) \par Note: cr\_mono\_mode \& cr\_fifo\_lr\_merge will be invalid in 3-channel mode \par Note: frame\_size must equal data\_size in 3/4/6-channel mode
\\\hline
...
0&i2s\_txf\_int&r&1'b1&I2S TX FIFO ready (tx\_fifo\_cnt > tx\_fifo\_th) interrupt, auto-cleared when data is pushed\\\hline
...
\regdes{31:28&RSVD& & & \\\hline
27:24&rx\_fifo\_th&r/w&4'd0&RX FIFO threshold, dma\_rx\_req will not be asserted if rx\_fifo\_cnt is less than this value\\\hline
23:20&RSVD& & & \\\hline
