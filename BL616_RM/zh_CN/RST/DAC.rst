...
DAC 数据格式
------------------
独立模式下DMA或者CPU向FIFO写入待转化数据时，FIFO 可以支持 10 种不同的存储格式，通过设置寄存器 gpdac_dma_config 中对应的 gpdac_dma_format 位数值，可以配置gpdac_dma_wdata 寄存器（宽度为32-bit）中存储的的数据传输格式，其对应关系如下：
