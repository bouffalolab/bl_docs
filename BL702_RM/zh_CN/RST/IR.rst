...
脉冲宽度发送
-------------
对于不适用于普通发送模式的协议，IR提供了脉冲宽度发送的方式。先计算出所使用的协议内各种不同宽度脉冲的公共脉冲宽度单位，即最大公约数，填入寄存器IRTX_PULSE_WIDTH的低12位。然后将从第一个电平开始到最后一个电平为止的各个电平宽度所对应的倍数填入寄存器IRTX_SWM_PW_n(0<=n<=7)中，每个电平宽度倍数占4-bit。
